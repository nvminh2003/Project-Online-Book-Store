...
const moveToCart = async (req, res) => {
    try {
        ...
        if (existingCartItem) {
            const maxAddable = book.stockQuantity - existingCartItem.quantity;
            if (maxAddable <= 0) {
                return res.status(400).json({
                    message: "Bạn đã có số lượng tối đa sản phẩm này trong giỏ hàng",
                    status: "Error",
                });
            }
            const addQuantity = Math.min(quantity, maxAddable);
            existingCartItem.quantity += addQuantity;
            // Nếu addQuantity < quantity, báo cho FE biết chỉ thêm được bấy nhiêu
            if (addQuantity < quantity) {
                await cart.save();
                // Remove from wishlist như thường lệ
                const wishlist = await Wishlist.findOne({ user: req.account._id });
                if (wishlist) {
                    wishlist.books = wishlist.books.filter(
                        (item) => !item.book || item.book.toString() !== bookId
                ...
                return res.status(200).json({
                    message: `Chỉ thêm được ${addQuantity} sản phẩm vào giỏ hàng (tối đa tồn kho).`,
                    status: "Warning",
                    data: {
                        bookId: book._id,
                        title: book.title,
            ...
            const addQuantity = Math.min(quantity, book.stockQuantity);
            cart.items.push({ book: bookId, quantity: addQuantity });
        }
...
const removeMultipleFromWishlist = async (req, res) => {
    try {
        const { bookIds } = req.body; // array of bookId
        if (!Array.isArray(bookIds) || bookIds.length === 0) {
            return res.status(400).json({ message: "Book IDs are required", status: "Error" });
        }
        const wishlist = await Wishlist.findOne({ user: req.account._id });
        if (!wishlist) {
            return res.status(404).json({ message: "Wishlist not found", status: "Error" });
        }
        wishlist.books = wishlist.books.filter(
            (item) => !item.book || !bookIds.includes(item.book.toString())
        );
        await wishlist.save();
        res.status(200).json({
            message: "Removed selected books from wishlist successfully",
            status: "Success",
            data: { removedBookIds: bookIds, totalWishlistItems: wishlist.books.length },
        });
    } catch (error) {
