...
const sendOrderConfirmationEmail = async (email, order) => {
  ...
  const itemsHtml = order.items
    .map(
      (item) => `
        <tr>
            <td style="padding:8px 4px;border:1px solid #eee;">${
              item.book?.title || ""
            }</td>
            <td style="padding:8px 4px;border:1px solid #eee;text-align:center;">${
              item.quantity
            }</td>
            <td style="padding:8px 4px;border:1px solid #eee;text-align:right;">${item.price.toLocaleString(
              "vi-VN"
            )}đ</td>
        </tr>
    `
    ...
    html: `
            <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;">
                ...
                <div style="margin-top:16px;text-align:right;">
                    ...
                    <p style="font-size:1.2em;"><b>Tổng thanh toán:</b> ${(
                      order.totalAmount || 0
                    ).toLocaleString("vi-VN")}đ</p>
                </div>
                <hr>
      ...
      <p><a href="${loginUrl}" style="display: inline-block; padding: 10px 15px; background-color: #007bff; color: white; text-decoration: none; border-radius: 4px;">Đăng nhập vào tài khoản</a></p>
                <p style="margin-top:24px;">Nếu bạn có bất kỳ thắc mắc nào, vui lòng liên hệ <a href="mailto:${
                  process.env.EMAIL_USER
                }">${process.env.EMAIL_USER}</a> để được hỗ trợ.</p>
...
const sendOrderCancellationEmail = async (email, order) => {
  ...
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: email,
    subject: `Đơn hàng #${order.orderCode} đã bị hủy - Book Store`,
    html: `
            <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;">
                <h2 style="color:#EF4444;">Đơn hàng của bạn đã bị hủy</h2>
                <p>Xin chào <b>${order.fullName}</b>,</p>
                <p>Chúng tôi xin thông báo đơn hàng #${
                  order.orderCode
                } của bạn đã bị hủy do thanh toán không thành công.</p>
                
                <h3>Thông tin đơn hàng đã hủy</h3>
                <ul>
                    <li><b>Mã đơn hàng:</b> ${order.orderCode}</li>
                    <li><b>Ngày đặt:</b> ${orderDate}</li>
                    <li><b>Trạng thái:</b> Đã hủy</li>
                    <li><b>Lý do:</b> Thanh toán không thành công hoặc bị hủy bởi khách hàng</li>
                </ul>
                ...
                <div style="margin-top:16px;text-align:right;">
                    <p style="font-size:1.2em;"><b>Tổng giá trị đơn hàng đã hủy:</b> ${(
                      order.totalAmount || 0
                    ).toLocaleString("vi-VN")}đ</p>
                </div>

                <div style="background:#FEF3C7;border:1px solid #F59E0B;padding:12px;margin:16px 0;border-radius:4px;">
                    <p style="margin:0;color:#92400E;"><b>Lưu ý:</b> Nếu bạn đã thanh toán, số tiền sẽ được hoàn lại trong vòng 3-5 ngày làm việc.</p>
                </div>

                <p>Bạn có thể đặt lại đơn hàng bằng cách:</p>
                <ol>
                    <li>Đăng nhập vào tài khoản của bạn</li>
                    <li>Thêm sản phẩm vào giỏ hàng</li>
                    <li>Chọn phương thức thanh toán khác hoặc thử lại</li>
                </ol>

                <p><a href="${loginUrl}" style="display: inline-block; padding: 10px 15px; background-color: #007bff; color: white; text-decoration: none; border-radius: 4px;">Đăng nhập để đặt lại</a></p>
                
                <p style="margin-top:24px;">Nếu bạn có bất kỳ thắc mắc nào về việc hủy đơn hàng hoặc hoàn tiền, vui lòng liên hệ <a href="mailto:${
                  process.env.EMAIL_USER
                }">${process.env.EMAIL_USER}</a> để được hỗ trợ.</p>
                <p style="color:#4F46E5;">Book Store xin lỗi vì sự bất tiện này!</p>
            </div>
        `,
  };
