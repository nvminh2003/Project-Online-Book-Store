...
import { validateDiscountCodeAPI } from "../services/discountService";
...
export const CartProvider = ({ children }) => {
  ...
  // Hàm validate discount code (không apply)
  const validateCoupon = useCallback(
    async (couponCode) => {
      ...
      try {
        ...
        const res = await validateDiscountCodeAPI(couponCode, items);
  ...
  const removeCoupon = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await removeCouponFromCartAPI();
      setCart(res.data.data);
  ...
  const discountAmount = useMemo(() => {
    if (!cart || !cart.couponDetails) return 0;
    return cart.couponDetails.discountAmountCalculated || 0;
  }, [cart]);

  // Check if has active coupon
  const hasActiveCoupon = useMemo(() => {
    return !!(cart && cart.couponDetails && cart.couponDetails.code);
  }, [cart]);
