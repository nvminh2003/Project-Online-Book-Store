...
const Orders = () => {
  ...
  const handleUpdateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      notifyError(
        (err && err.message) || "Cập nhật trạng thái đơn hàng thất bại"
      );
  ...
  const handleUpdatePaymentStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      notifyError(
        (err && err.message) || "Cập nhật trạng thái thanh toán thất bại"
      );
  ...
  const columns = [
    { key: "orderCode", label: "Mã đơn" },
    {
      ...
      render: (order) =>
        order.fullName ||
        order.user?.customerInfo?.fullName ||
        order.user?.email,
    },
    { key: "phone", label: "Số điện thoại" },
    {
      key: "totalAmount",
      label: "Tổng tiền",
      render: (order) => order.totalAmount?.toLocaleString() + " đ",
    },
    {
      key: "orderStatus",
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {selectedOrder.fullName ||
                    selectedOrder.user?.customerInfo?.fullName ||
