...
const AddBook = () => {
  ...
  const uploadImagesToCloudinary = async () => {
    const uploadPromises = images.map((image) => {
      ...
      formData.append('upload_preset', 'book_upload'); // đúng preset
      return axios.post('https://api.cloudinary.com/v1_1/dhwegqmxl/image/upload', formData); // đúng cloud_name
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Tạm comment vì chưa có login
      // const token = localStorage.getItem("token");
      // const res = await axios.post(`${apiUrl}/books`, payload, {
