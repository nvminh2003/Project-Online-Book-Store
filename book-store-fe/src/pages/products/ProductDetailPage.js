    ...
    <div className="p-6 max-w-5xl mx-auto">
          ...
          <div className="relative bg-white rounded-2xl shadow-lg p-4 flex flex-col items-center">
            ...
            <div className="flex gap-4 items-center">
              <button
                ...
                onClick={() => setCurrentImageIdx((prev) => (prev - 1 + book.images.length) % book.images.length)}
              ...
              <button
                className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300 text-lg"
                onClick={() => setCurrentImageIdx((prev) => (prev + 1) % book.images.length)}
                disabled={book.images.length <= 1}
              >
                &#8594;
              </button>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="w-full md:w-1/3">
          <div className="relative cursor-pointer" onClick={() => { setShowImageModal(true); setCurrentImageIdx(0); }}>
            <img
            ...
            {book.images?.length > 1 && (
              <span className="absolute bottom-2 right-2 bg-black bg-opacity-60 text-white text-xs px-2 py-0.5 rounded">Xem tất cả ảnh</span>
            )}
          </div>
          {book.images?.length > 1 && (
            <div className="mt-4 grid grid-cols-4 gap-2">
              {book.images.slice(1).map((img, idx) => (
                <img
                  ...
                  onClick={() => { setShowImageModal(true); setCurrentImageIdx(idx + 1); }}
                />
              ))}
            </div>
          )}
